...
var server = net.createServer(function (socket) {
	...
	socket.on('data', function(usrData){
		...
		if(usrData.length>=4){
			...
			commands.commandHandle(cNum, socket.usr, hashMap, usrData, rooms);
		}
	});	
	//when client closes app
	socket.on('end', function(){
		console.log("exit");
		room.removePlayer(rooms, socket.usr.room, socket.usr);
		var i=usrs.indexOf(socket.usr);
		usrs.splice(i,1);
	});
});
//server.listen(843, "127.0.0.1");
server.listen(843, "192.168.4.101");

setInterval(runRooms, 200);
